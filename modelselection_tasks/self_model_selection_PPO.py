...
class Args:
    ...
    hparam_to_tune: str = "self"
    # num_base_learners: int = 5
    # base_learners_hparam = [9, 33, 101, 512, 906]
    # base_learners_hparam = [7, 33, 101, 512, 1013]
    num_base_learners: int = 1
    base_learners_hparam = [408]
    ...
    wandb_project_name: str = f"Modsel_PPO_Self"
...
def normalize_episodic_return(episodic_return, normalizer_const, args):
    if args.env_id == "Acrobot-v1":
        ...
        normalized_return = (episodic_return + 600) / 1200
    elif args.env_id == "Ant-v4":
        normalized_return = (episodic_return + 1000) / 5000
    elif args.env_id == "Pusher-v4":
        normalized_return = (episodic_return + 200) / 200
    else:
...
class BaseLearner(nn.Module):
    def __init__(self, base_index, envs, device, args):
        ...
        # Set seed for this specific base learner if provided
        if args.hparam_to_tune == "self":
            torch.manual_seed(args.base_learners_hparam[base_index])
            np.random.seed(args.base_learners_hparam[base_index])
            random.seed(args.base_learners_hparam[base_index])
        ...
        # Reset global seed back to original after base learner initialization
        if args.hparam_to_tune is "self":
    ...
    def increment_internal_time(self):
...
if __name__ == "__main__":
    ...
    run_name = f"{args.modsel_alg}__{args.env_id}__{args.exp_name}__{int(time.time())}"
    if args.track:
        import wandb
        wandb.init(
            ...
            if "final_info" in infos:
                for info in infos["final_info"]:
                    if info and "episode" in info:
                        ...
                        normalized_return = normalize_episodic_return(episodic_return, normalizer_const=5000, args=args)
                        modsel.update_distribution(base_index, normalized_return)
                        agent.increment_internal_time()
                        
                        writer.add_scalar("modelselection/MetaLearner", normalized_return, global_step)
                        writer.add_scalar(f"baselearners/seed={args.base_learners_hparam[base_index]}", normalized_return, global_step)
                        writer.add_scalar(f"baselearners/internal_time_seed={args.base_learners_hparam[base_index]}", agent.get_internal_time(), global_step)
